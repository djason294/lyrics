...
<?php
	...
	*/
	...
	$f = fopen("groups_album_song.txt", "r");
	$num=2;
	...
	while(!feof($f)) {
		...
		if(startsWith($str,"singer")){
			...
			echo $str_sql."<br/>";
	  		...
	  		$str_sql="INSERT INTO song (albumID,songTitle,songUrl,songSinger, lyricWriter,musicWriter,lyric,type) values ('"
	  			.trim($albumID)."','"
	  			.trim($songName)."','"
	  			.trim($songLink)."','"
	  			.trim($albumSinger)."', '"
