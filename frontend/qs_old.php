...
<body>
	...
	<?php
		...
		// 查詢是否有這個歌手
		$sql = 'select songYear from song where song.songSinger = "' . $q . '"';// GROUP BY songYear ORDER BY songYear';
		//$sql = 'select * from album';
		//echo $sql.'<br>';
		$result = mysqli_query($linkid,$sql);
		$resultarray = mysqli_fetch_array($result);
		...
		for(;$tempY<=2016;$tempY++){
			$tempYS = strval($tempY);
			if(!array_key_exists($tempYS,$r)){
				$r[$tempYS] = 0;
			}
		}
		while($resultarray!=null){
			if(array_key_exists(substr($resultarray[0],0,4),$r))
				$r[substr($resultarray[0],0,4)] += 1;
			else
				$r[substr($resultarray[0],0,4)] = 1;
			if($max<$r[substr($resultarray[0],0,4)])
				$max=$r[substr($resultarray[0],0,4)];
			$resultarray = mysqli_fetch_array($result);	
		}
		// to new 
		$index=0;
